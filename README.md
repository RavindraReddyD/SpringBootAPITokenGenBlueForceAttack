 ...
 * Non-admin username and password: Ravindra.Reddy and jwtpass
 * Admin user: admin.admin and jwtpass
 * Example of resource accessible to all authenticated users:  http://localhost:9080/springjwt/cities
 * Example of resource accessible to only an admin user:  http://localhost:9080/springjwt/users

 1. Generate an access token

   Use the following generic command to generate an access token:
   `$ curl client:secret@localhost:9080/oauth/token -d grant_type=password -d username=user -d password=pwd`

   For this specific application, to generate an access token for the non-admin user Ravindra.Reddy, run:
   `$ curl testjwtclientid:XY7kmzoNzl100@localhost:9080/oauth/token -d grant_type=password -d username=Ravindra.Reddy -d password=jwtpass`
